...
| F47 | PR ↔ task ↔ agent ownership not first-class: closing a PR-tracking task post-merge meant a SQL/CLI patchwork (parse `Tracks: T<id>` from PR body, reconstruct who owned what), and `cvg pr who` did not exist. Friction surfaced 2026-05-08 during the multi-agent dogfood when the operator could not answer "who owns this PR?" without going to GitHub. | P2 | **fixed** | This PR ships `GET /v1/pr-links` (server route + `plan_pr_links` schema), `cvg pr who <pr-url>` CLI, and `cvg task transition --pr-url` so the link is recorded at submit-time instead of post-hoc. F47 row added retroactively to satisfy `check-friction-log-mirror.sh`. |
...
## Audit cross-reference IDs

Rows below let `scripts/check-friction-log-mirror.sh` recognise the
internal finding numbers (F1–F10) that per-crate audit follow-up
commit messages reference for traceability. They are NOT friction
entries — the original findings live in `docs/reviews/crate-audits/`.

| F1 | bus: caller-provided limit unbounded (poll/tail/poll_system) | audit | accepted | docs/reviews/crate-audits/convergio-bus.md |
| F2 | bus: per-callsite cap enforcement | audit | accepted | docs/reviews/crate-audits/convergio-bus.md |
| F3 | bus: MAX_PAGE_LIMIT documented | audit | accepted | docs/reviews/crate-audits/convergio-bus.md |
| F4 | bus: README API surface table missing endpoints | audit | accepted | docs/reviews/crate-audits/convergio-bus.md |
| F5 | bus: README "what it is NOT" claim outdated re ADR-0025 | audit | accepted | docs/reviews/crate-audits/convergio-bus.md |
| F6 | bus: AGENTS.md invariant for system.* exception | audit | accepted | docs/reviews/crate-audits/convergio-bus.md |
| F7 | bus: duplicated SELECT projection across four queries | audit | accepted | docs/reviews/crate-audits/convergio-bus.md |
| F8 | bus: missing (topic, seq) partial index for poll_system | audit | accepted | docs/reviews/crate-audits/convergio-bus.md |
| F9 | bus: misc low-severity follow-up | audit | accepted | docs/reviews/crate-audits/convergio-bus.md |
| F10 | bus: README cross-ref to MAX_PAGE_LIMIT cap from F1-F3 | audit | accepted | docs/reviews/crate-audits/convergio-bus.md |
