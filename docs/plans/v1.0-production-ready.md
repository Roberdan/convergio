...
>
> **Ship status (2026-05-27)**: W1–W11 **shipped on `main`**
> (W11 wire-up via #455 + crate stub #454, ADR-0064). Only **W12**
> (`convergio-edu` demo, separate repo) remains.
...
| # | Workstream | Stima (gg) | Dipende da | Priorità | Status |
|---|---|---|---|---|---|
| W1 | `A11yGate` phase 1 (built-in, no axe) | 4-5 | — | **bloccante v1.0** | ✅ shipped |
| W2 | `PromptInjectionGate` | 3-4 | — | **bloccante v1.0** | ✅ shipped |
| W3 | Smart Thor — esecuzione pipeline reale | 6-8 | — | **bloccante v1.0** | ✅ shipped |
| W4 | OKR schema + `PlanCoherenceGate` + `PlanOutcomeGate` | 5-7 | W3 | **bloccante v1.0** | ✅ shipped |
| W5 | Wave 0b — `/cvg-attach` hooks completi + `cvg status --agents` | 6-9 | — | importante | ✅ shipped |
| W6 | Plan template parametrici (`vertical-accelerator-v1`) | 4-6 | W4 | importante | ✅ shipped |
| W7 | `ai-openai` runner adapter | 3-4 | — | importante | ✅ shipped |
| W8 | Multi-vendor routing in `cvg dispatch` | 5-7 | W7 | importante | ✅ shipped |
| W9 | Remote capability registry (HTTPS + mirror) | 8-12 | — | importante | ✅ shipped |
| W10 | Model evaluation framework (Cost-of-Pass) | 7-10 | W3, W8 | importante | ✅ shipped |
| W11 | `A11yGate` phase 2 (axe-core capability `a11y.axe`) | 5-7 | W1, W9 | **bloccante v1.0** (W12 demo dependency) | ✅ shipped (#454 + #455) |
| W12 | `convergio-edu` vertical accelerator demo | 10-15 | W4, W6, W9 | **demo v1.0 launch** | ⏭ separate repo |
...
### W11 — `A11yGate` phase 2 (capability `a11y.axe`) — ✅ shipped

**Cosa manca**: integrazione axe-core. Phase 1 (W1) copre i check
testuali; phase 2 copre evidence UI (`html_output`, `screenshot`,
`component_render`).

**Cosa fatto** (PR #454 + #455, ADR-0064):
- Crate leaf `convergio-a11y-axe` con wrapper opt-in via
  `CONVERGIO_A11Y_AXE_BIN` (no Node dep nel daemon — il binary
  axe lo fornisce l'operatore).
- `A11yGate` estende phase-1: quando `kind ∈ {html_output, html,
  component_render}` e binary è configurato, dispatch a
  `convergio_a11y_axe::run_html`.
- Refuse a severity ∈ `{serious, critical}` come
  `a11y_violation_found: {kind}#axe:{rule_id}, …` (stesso wire shape
  di phase-1, nessuna API churn).

**Definition of Done**:
- [x] Crate stub merged + wire-up merged.
- [x] Default-off: senza env var il comportamento è identico a
      phase-1 (run_html → `NotConfigured`).
- [ ] Capability install via `cvg capability install a11y.axe` —
      deferred al primo bundle pubblicato sul registry remoto (W9)
      che impacchetti uno script axe-core "ready to run".
