...
/// One plan entry returned by `/v1/plans`. Public to the parent
/// module so the renderer can format it without re-cloning.
...
pub(super) struct SessionReport<'a> {
    ...
    pub(super) heartbeat: Value,
    /// Active plans the agent is now visible on.
    pub(super) plans: &'a [PlanRef],
    /// `(plan_id, messages)` for direct (`agent:<id>`) traffic.
    pub(super) direct: &'a [(String, Vec<Value>)],
    /// `(plan_id, messages)` for plan-wide (`plan:<id>`) traffic.
    pub(super) announcements: &'a [(String, Vec<Value>)],
}

/// Forward to the rendering module. Kept here as a thin wrapper so
/// callers depending on this module don't have to know about the
/// sibling module split.
fn render(output: OutputMode, bundle: &Bundle, report: &SessionReport<'_>) -> Result<()> {
    super::session_register_and_poll_render::render(output, bundle, report)
}
