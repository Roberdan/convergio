...
pub fn build(inputs: &PromptInputs<'_>) -> String {
    ...
    s.push_str("- Pre-push hooks may refuse a push when derived files are stale. The two known triggers in this repo are `docs/INDEX.md` (run `./scripts/generate-docs-index.sh`) and the `<!-- BEGIN AUTO:... -->` blocks (run `cvg docs regenerate --root .`). If a hook complains about either, run the regenerator, commit the diff with a `chore(repo):` message, and retry the push — these maintenance commits are explicitly allowed, even when a task description forbids \"modifying any other file\".\n");
